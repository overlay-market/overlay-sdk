...
export class OverlaySDKTrade extends OverlaySDKModule {
    ...
    async getUnwindState(marketAddress, account, posId, unwindPercentage, // 0.01 - 1
    slippage, decimals) {
        const chainId = this.core.chainId;
        invariant(chainId in CHAINS, "Unsupported chainId");
        ...
        if (!positionDetails)
            return { error: "Position not found", isShutdown: false, cost: 0, unwindState: UnwindState.PositionNotFound };
        ...
        const { cost, currentOi, info, liquidatePrice, marketMid, positionValue, debt, collateral, notional, maintenanceMargin, prices } = await this.sdk.openPositions.getOpenPositionData(chainId, account, marketAddress, positionId);
        ...
        let unwindState = UnwindState.Unwind;
        if (1 < unwindPercentage)
            unwindState = UnwindState.UnwindAmountTooHigh;
        if (0.01 > unwindPercentage)
            unwindState = UnwindState.PercentageBelowMinimum;
